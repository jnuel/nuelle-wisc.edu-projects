...
for i = 1:length(img_list)
    img = imread(['edge_' img_list{i} '.png']);
     H = generateHoughAccumulator(img, theta_num_bins, rho_num_bins);
    ...
    imwrite(uint8(H), ['accumulator_' img_list{i} '.png']);
end
...
img_list = {'hough_1', 'hough_2', 'hough_3'};
% hough_threshold = 0.5 * max(hough_img(:)); Started from here
hough_threshold = [128, 128, 135];
