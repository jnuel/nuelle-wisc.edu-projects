...
%     assert(isa(wrapped_imgs, 'uint8') | isa(wrapped_imgs, 'single') | isa(wrapped_imgs, 'double'));
    ...
    for c = 1:Cs
        ...
        elseif strcmp(mode, 'blend')
            ...
            % ADD YOUR CODE HERE
            % ---------------------------
            % first invert the masks and apply bwdist using method euclidean to compute new
            % weighted mask
            weighted_masks = bwdist(~masks, 'euclidean');
            weighted_maskd = bwdist(~maskd, 'euclidean');
            % then convert the masks to same dimensions so we can multiply
            % for RGB
            ...
            % blend the two images based off their weights and weightd,
            % then assign to out_img
            weighted_wrapped_imgs = wrapped_imgs .* weighted_masks;
            weighted_wrapped_imgd = wrapped_imgd .* weighted_maskd;
            % out_img assigned
            out_img = (weighted_wrapped_imgs + weighted_wrapped_imgd)./(weighted_masks + weighted_maskd);
